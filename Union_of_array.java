...
public class Union_of_array {
    public static void unionofTwoSortedArrays(int[] arr1,int[] arr2,List<Integer> finalList){
        int i=0,j=0;
        ...
        while (j < arr2.length) {
            if (j<arr2.length-1 && arr2[i] == arr2[j+1]){
                j++;
            }else{
                finalList.add(arr2[j++]);
            }
    ...
    public static void main(String[] args) {
        int[] arr1 = {2,3,4,5,5,5,7,7,8};
        int[] arr2 = {1,2,2,2,4,6,6,6,7,8,10};
        List<Integer> finalUnion = new ArrayList<>();
        unionofTwoSortedArrays(arr1,arr2,finalUnion);
